    ...
    <div data-testid="therapist-dashboard" className="space-y-6 lg:space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-6">
        ...
        <div className="space-y-5 lg:space-y-6">
          ...
          {/* Alerts & Tasks - PRIORITY SECTION */}
          {alerts.length > 0 && (
            <Card className="p-5 bg-gradient-to-r from-amber-50/50 to-orange-50/50 border-amber-200">
              <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                <Bell size={18} className="text-amber-500" />
                Needs Attention
                <span className="ml-2 px-2 py-0.5 bg-amber-500 text-white text-xs rounded-full">{alerts.length}</span>
              </h3>
              ...
              <div className="space-y-3">
                {alerts.map((alert, idx) => {
                  const Icon = alert.icon;
                  // Different bright colors for each alert type
                  const colorStyles = alert.isNewRegistration 
                    ? 'bg-gradient-to-r from-emerald-100 to-teal-100 border-emerald-400 border-2'
                    : alert.type === 'warning' 
                      ? 'bg-gradient-to-r from-amber-100 to-yellow-100 border-amber-400 border-2' 
                      : alert.type === 'error'
                        ? 'bg-gradient-to-r from-red-100 to-rose-100 border-red-400 border-2'
                        : alert.type === 'success'
                          ? 'bg-gradient-to-r from-green-100 to-emerald-100 border-green-400 border-2'
                          : 'bg-gradient-to-r from-blue-100 to-indigo-100 border-blue-400 border-2';
                  
                  const iconColor = alert.isNewRegistration 
                    ? 'text-emerald-600'
                  ...
                  return (
                    <div
                      key={idx}
                      className={`p-4 rounded-xl flex items-start gap-3 shadow-sm hover:shadow-md transition-shadow ${colorStyles}`}
                      data-testid={`alert-${idx}`}
                    >
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        alert.isNewRegistration ? 'bg-emerald-200' :
                        alert.type === 'warning' ? 'bg-amber-200' :
                        alert.type === 'error' ? 'bg-red-200' :
                        alert.type === 'success' ? 'bg-green-200' : 'bg-blue-200'
                      }`}>
                        <Icon size={16} className={iconColor} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium ${
                          alert.isNewRegistration ? 'text-emerald-800' :
                        ...
                        {alert.action && !isReadOnly && (
                          <Button 
                            variant="link" 
                            size="sm" 
                            className={`p-0 h-auto text-sm mt-1 font-semibold ${
                              alert.isNewRegistration ? 'text-emerald-700 hover:text-emerald-900' :
                              alert.type === 'warning' ? 'text-amber-700 hover:text-amber-900' :
                              alert.type === 'error' ? 'text-red-700 hover:text-red-900' :
                              alert.type === 'success' ? 'text-green-700 hover:text-green-900' : 'text-blue-700 hover:text-blue-900'
                            }`}
