    ...
    <div data-testid="therapist-dashboard" className="space-y-8">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">{formatDateLong(nowIST())}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card 
          className={`p-5 border-l-4 ${stats.todayAppointments > 0 ? 'border-l-primary bg-primary/5' : 'border-l-muted bg-muted/30'} cursor-pointer hover:shadow-md transition-all`}
          onClick={() => handleNavigate('appointments')}
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <div className={`p-2.5 rounded-full ${stats.todayAppointments > 0 ? 'bg-primary/10' : 'bg-muted'}`}>
              <Calendar size={22} className={stats.todayAppointments > 0 ? 'text-primary' : 'text-muted-foreground'} />
            </div>
          </div>
          {stats.todayAppointments === 0 && !isReadOnly && (
            <Button 
              variant="link" 
              size="sm" 
              className="mt-3 p-0 h-auto text-primary"
              onClick={(e) => { e.stopPropagation(); handleNavigate('appointments'); }}
            >
        ...
        {/* Messages Card */}
        <Card 
          className={`p-5 border-l-4 ${stats.unreadMessages > 0 ? 'border-l-blue-500 bg-blue-50' : 'border-l-muted bg-muted/30'} cursor-pointer hover:shadow-md transition-all`}
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <div className={`p-2.5 rounded-full ${stats.unreadMessages > 0 ? 'bg-blue-100' : 'bg-muted'}`}>
              <MessageSquare size={22} className={stats.unreadMessages > 0 ? 'text-blue-500' : 'text-muted-foreground'} />
            </div>
          </div>
        </Card>
        ...
        {/* Pending Notes Card */}
        <Card 
          className={`p-5 border-l-4 ${stats.pendingNotes > 0 ? 'border-l-amber-500 bg-amber-50' : 'border-l-muted bg-muted/30'} cursor-pointer hover:shadow-md transition-all`}
          onClick={() => handleNavigate('notes')}
          data-testid="pending-notes-card"
        >
          <div className="flex items-start justify-between">
            <div>
              <p className="text-3xl font-bold text-foreground">{stats.pendingNotes}</p>
              <p className="text-sm text-muted-foreground font-medium">Pending Notes</p>
              {stats.pendingNotes > 0 ? (
                <p className="text-xs text-amber-600 mt-1.5 font-medium">
                  Sessions need documentation
                </p>
              ) : (
                <p className="text-xs text-success mt-1.5 flex items-center gap-1">
            ...
            <div className={`p-2.5 rounded-full ${stats.pendingNotes > 0 ? 'bg-amber-100' : 'bg-muted'}`}>
              <FileText size={22} className={stats.pendingNotes > 0 ? 'text-amber-500' : 'text-muted-foreground'} />
            </div>
          </div>
          {stats.pendingNotes > 0 && !isReadOnly && (
            <Button 
              variant="link" 
              size="sm" 
              className="mt-3 p-0 h-auto text-amber-600"
              onClick={(e) => { e.stopPropagation(); handleNavigate('notes'); }}
            >
              Add notes <ArrowRight size={14} className="ml-1" />
            </Button>
          )}
        </Card>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="p-4">
              {todaySchedule.length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">Your schedule is clear for today</p>
                  {!isReadOnly && (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            {appt.status === 'completed' ? (
                              ...
                              <span className="px-3 py-1 bg-primary text-white text-xs rounded-full font-medium animate-pulse">
                                In Session
                              </span>
                            ) : isNow ? (
          ...
          <Card className="p-6 bg-white/70 backdrop-blur-xl border border-border/40">
            ...
            {weekSchedule.length === 0 ? (
              <p className="text-center py-6 text-muted-foreground">No appointments this week</p>
            ) : (
              <div className="space-y-3">
                {weekSchedule.slice(0, 5).map(([dateKey, appts]) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-lg flex flex-col items-center justify-center text-center ${isToday ? 'bg-primary text-white' : 'bg-muted'}`}>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {appts.length > 3 && (
                          <span className="text-xs text-muted-foreground ml-1">+{appts.length - 3}</span>
                        )}
                      </div>
                    </div>
                  );
          ...
          {nextAppointment && (
            <Card className="overflow-hidden border-2 border-primary/20">
              <div className="bg-gradient-to-r from-primary to-primary/80 px-5 py-4 text-white">
                ...
                <p className="text-sm opacity-90">{getTimeUntil(nextAppointment.start_time)}</p>
              </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {alert.action && !isReadOnly && (
                          ...
                          >
                            {alert.action} <ArrowRight size={12} className="ml-1" />
                          </Button>
                        )}
                      </div>
                    </div>
