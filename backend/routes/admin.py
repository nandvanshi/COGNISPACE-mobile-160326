...
async def approve_therapist(application_id: str, background_tasks: BackgroundTasks, password: Optional[str] = None, current_user: dict = Depends(require_super_admin)):
    ...
    password_for_email = None  # Only send password in email if it was auto-generated by admin
    if application.get("password_hash"):
        password_hash = application["password_hash"]
        # Therapist set their own password, don't send it in email
    elif password:
        password_hash = hash_password(password)
        password_for_email = password  # Send admin-generated password in email
    else:
    ...
    background_tasks.add_task(
        send_therapist_welcome_notifications,
        therapist_name=application["full_name"],
        email=application.get("email"),
        mobile=application["mobile"],
        password=password_for_email,
        trial_end_date=trial_end.strftime("%d %b %Y")
    )
    
    return {"message": "Therapist approved successfully", "therapist_id": therapist_id}


async def send_therapist_welcome_notifications(
    therapist_name: str,
    email: Optional[str],
    mobile: str,
    ...
    """Send welcome email and WhatsApp to newly approved therapist"""
